...
import { writeConfigV2 } from '../config/load.js';
...
function buildMigratedConfig(config: LlmDocsConfigV2): LlmDocsConfigV2 {
  return {
    ...config,
    workspace: {
      ...config.workspace,
...
export async function applyActiveTasksMigration(
  ...
  await writeConfigV2(projectRoot, migratedConfig);
