...
import { loadConfig, loadOrMigrateConfig } from '../../src/engine/config/load.js';
import { migrateLegacyConfig } from '../../src/engine/config/migrate.js';
import { ConfigMigrationError } from '../../src/engine/errors.js';
import { makeTempProject } from './test-helpers.js';

describe('config', () => {
  it('migrates a legacy config into the expected current shape', async () => {
  ...
  it('creates a new default config when missing', async () => {
    const projectRoot = await makeTempProject();
    ...
    const persisted = await loadConfig(projectRoot);
  ...
  it('migrates and writes a legacy config file', async () => {
    const projectRoot = await makeTempProject();
